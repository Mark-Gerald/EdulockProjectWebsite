    ...
    return new Promise((resolve, reject) => {
        database.ref(`registered_devices/${deviceId}`).once('value')
            .then(snapshot => {
                ...
                // Mark online immediately — also remove from offline set in case
                // this device was previously seen as offline in this session
...
// Mark a device as offline — adds to the persistent offline set
function markDeviceOffline(deviceId) {
    if (deviceStatusMap[deviceId] === 'online') {
        deviceStatusMap[deviceId] = 'offline';
        sessionOfflineSet.add(deviceId);
        updateDeviceStatusUI(deviceId, 'offline');
        logConnectionEvent(deviceId, Date.now(), 'disconnected');
        updateDeviceCounts();
 ...
 */
function logConnectionEvent(deviceId, timestamp, eventType) {
    ...
    eventEl.textContent = eventType === 'disconnected' ? 'Disconnected' : 'Connected';
